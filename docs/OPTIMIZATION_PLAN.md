...
- **H3 Tiered JIT (interpreter-driven hot-spot compilation)** · **[P0 + P1 + P2 + P3a infrastructure landed, Linux x86_64 only]**:
  ...
  - **H3-P3a — Unbalanced JIT infrastructure with balanced-only gating** · **[landed; eligibility temporarily tightened]**:
    - **ABI extension (commit `ccdffab`)**: `compile_lir_to_jit_loop_asm` / `compile_lir_to_jit_asm` switched to a SysV 16-byte struct return (rax=status / rdx=final data_ptr); `crates/jit/src/lib.rs::JitExit` + `execute_loop_fn` followed. This unblocks unbalanced loops handing the post-iteration `data_ptr` back to the host.
    - **Per-iteration bounds check + bail trampoline**: `compile_lir_to_jit_loop_asm` now takes `iter_check: Option<(i32,i32)>`. With `Some((min_off, max_off))` it emits a head-of-loop check `lea r15, [r13+min_off]; cmp r15, r12; jb exit_one` and a symmetric upper-bound check, bailing with status=1 on out-of-range. `emit_ensure_tape_bail` rewires the AOT mmap+memcpy+munmap path to `add rsp, 8; jmp exit_one`, so the host's persistent scratch never gets unmapped under the JIT.
    - **`dispatch_jit` accepts bail-and-resume**: returns `Result<usize, RuntimeError>` carrying the next pc — status=0 (natural completion) → `loop_end_pc + 1`; status=1 (bail) → `start_pc as usize`, letting the interpreter run one more body iteration in slow mode before retrying the JIT against a freshly grown tape. Pre-grow on unbalanced paths adds 4 KiB of drift headroom (`UNBALANCED_HEADROOM`) to amortise bail frequency; balanced paths skip it.
    - **Eligibility temporarily restricted to balanced (`net_delta == 0`)**: `analyse_eligibility` still requires `ptr == 0`. Local matslina-suite measurements showed unbalanced-eligibility regressed mandelbrot O3 from 8.7 s to >30 s — the per-dispatch `snapshot_flat_into` + `restore_from_flat` memcpy pair dwarfs the 1-3 op bodies that typically appear in unbalanced loops. Closing the gap to the doc's projected 3-5× mandelbrot win requires the persistent mmap-shared tape (originally H3-P3) to remove the snapshot/restore cost altogether; until then, JIT'ing unbalanced loops is a net negative.
    - **New reach-span cap** (`ELIGIBILITY_REACH_SPAN_CAP = 1024`): reject any body that touches more than 1024 cells per iteration. Dispatch memcpy cost scales with visited span, so wide-span loops (e.g., 200 KiB drift loops) lose to the interpreter even when balanced.
    - **Local bench check (2026-04-29)**: mandelbrot O3 interpret 8.1 s / tiered 9.9 s (slightly better than the v2 10.97 s baseline thanks to the tighter JIT wrapper); hanoi O3 / long O3 within noise of interpret (55 ms / 147 ms). No regression.
    - Files: `src/interp/jit_compile.rs` (`LoopReach`, cap constant, rewritten `analyse_eligibility`), `src/interp/handlers.rs` (`dispatch_jit` returns next pc; bail handling), `src/interp/engine.rs` (`JitState::Ready { reach: LoopReach }`), `src/backend/codegen.rs` (`compile_lir_to_jit_loop_asm` `iter_check` parameter + self-managed wrapper + `emit_ensure_tape_bail`).
  - **H3-P3 (persistent mmap-Tape backend) deferred**: under the v2 baseline, even zero-cost dispatch bridging would only narrow the per-iteration JIT vs. interpreter gap from 30 ns to 50 ns; the matslina-suite ceiling for P3 alone is ~30% (150 ms → ~100 ms), out of proportion to the engineering cost. **However, P3 is now the prerequisite for unlocking H3-P3a's unbalanced eligibility** — once it lands, dropping the `net_delta == 0` gate opens the mandelbrot-tier win the doc projected.
  - **Next-step candidates (ROI-ordered)**:
    1. **H3-P3 persistent mmap-shared tape**: removes the snapshot/restore memcpy from the dispatch path; immediately enables relaxing the P3a `net_delta == 0` gate. The mandelbrot 3-5× ceiling is reachable only after this.
    2. **F5 incremental compile cache**: bench shows compile time isn't the bottleneck, weak ROI.

**Dependencies: H1, H2, H3 (P0+P1+P2 + Step 2A/2B + P3a infrastructure) landed. The tiered JIT baseline holds at parity with interpret; unbalanced eligibility is wired but gated, awaiting H3-P3.**
