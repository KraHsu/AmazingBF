...
- **D2 SIMD-specialized forms**:
  ...
  - **D2b Contiguous `ZeroRun(count ≥ 16)` → `rep stosb`**: threshold derived from the 16-17 B setup vs 5 N B scalar stores crossover.
  - **D2c `LinearMul` ±1 columns** → `add/sub byte [r13], bl` directly, skipping the imul.
