...
- **D3 Buffered I/O** · **[解释器 + Linux 后端已实现 / Windows 后端待落地]**：
  ...
  - **Windows 后端侧（待）**：`src/backend/x86_64/windows.rs` 的 `emit_put_byte` / `emit_get_byte` 仍走 `WriteFile` / `ReadFile` IAT 调用，每字节一次。后续 commit 按相同寄存器约定改造。同一个 PR 需附带两处连带修改：（1）`compile_lir_to_windows_program` 的 `LirInst::LinearMul` 分支（`windows.rs:309-324`）目前没有像 Linux 后端（`codegen.rs:294,304`）那样包 `push rbx / pop rbx`——因为当前 Windows 侧 `rbx` 是自由的；D3 把 `rbx` 固定为输出缓冲写指针后，必须补这对 push/pop 以保护循环体内的被钉寄存器。（2）`GetByte` 与正常退出路径都需要在阻塞 / 终止前发一次 `call flush_output`，与 Linux 在 `codegen.rs:483-497` 和 `codegen.rs:523-528` 的顺序保持一致。
