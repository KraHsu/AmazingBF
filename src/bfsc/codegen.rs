...
impl<'a> BfEmitter<'a> {
    fn new(layout: &'a MemMap) -> Self {
        BfEmitter { output: String::new(), ptr: 0, layout, used: BTreeSet::new() }
    }

    fn temp_base(&self) -> usize { self.layout.temp_base }
    ...
    fn talloc(&mut self) -> usize {
        let mut i = 0;
        while self.used.contains(&i) { i += 1; }
        self.used.insert(i);
        self.temp_base() + i
    }
    ...
    fn talloc_n(&mut self, n: usize) -> usize {
        ...
        loop {
            ...
            if ok {
                for k in 0..n { self.used.insert(i + k); }
    ...
    fn tfree_n(&mut self, base: usize, n: usize) {
        ...
        for k in 0..n { self.used.remove(&(start + k)); }
    }

    fn tsave(&self) -> BTreeSet<usize> { self.used.clone() }
    fn trestore(&mut self, s: BTreeSet<usize>) { self.used = s; }
    ...
    fn goto(&mut self, c: usize) {
        if c > self.ptr {
            for _ in 0..(c - self.ptr) { self.output.push('>'); }
        } else if c < self.ptr {
            for _ in 0..(self.ptr - c) { self.output.push('<'); }
        }
        self.ptr = c;
    }

    fn clear(&mut self, c: usize) { self.goto(c); self.raw("[-]"); }
    fn inc(&mut self, c: usize, n: usize) { if n > 0 { self.goto(c); for _ in 0..n { self.output.push('+'); } } }
    fn dec(&mut self, c: usize, n: usize) { if n > 0 { self.goto(c); for _ in 0..n { self.output.push('-'); } } }

    fn setv(&mut self, c: usize, v: u8) {
        self.clear(c);
        if v > 0 { self.inc(c, v as usize); }
    }
    ...
    fn inp(&mut self, c: usize) { self.goto(c); self.output.push(','); }
    fn out(&mut self, c: usize) { self.goto(c); self.output.push('.'); }

    // non-destructive copy src → dst (uses one internal temp)
    fn copy(&mut self, src: usize, dst: usize) {
        let tmp = self.talloc();
        self.clear(dst); self.clear(tmp);
        self.goto(src); self.raw("[");
        self.goto(dst); self.output.push('+');
        self.goto(tmp); self.output.push('+');
        self.goto(src); self.raw("-]");
        self.goto(tmp); self.raw("[");
        self.goto(src); self.output.push('+');
        self.goto(tmp); self.raw("-]");
        self.tfree(tmp);
    }

    fn copy_n(&mut self, src: usize, dst: usize, n: usize) {
        for i in 0..n { self.copy(src + i, dst + i); }
    }
    ...
    fn bfmove_n(&mut self, src: usize, dst: usize, n: usize) {
        for i in 0..n { self.bfmove(src + i, dst + i); }
    }
    ...
    fn is_nz(&mut self, cell: usize) -> usize {
        let r = self.talloc(); self.clear(r);
        let tc = self.talloc();
        self.copy(cell, tc);
        self.goto(tc); self.raw("[");
        self.setv(r, 1);
        self.clear(tc);
        self.goto(tc); self.raw("]");
        self.tfree(tc);
    ...
    where F: FnOnce(&mut Self), G: FnOnce(&mut Self) {
        match else_fn {
            None => {
                self.goto(cond); self.raw("[");
                then_fn(self);
                self.clear(cond); self.goto(cond); self.raw("]");
            }
            Some(ef_fn) => {
                let ef = self.talloc(); self.setv(ef, 1);
                self.goto(cond); self.raw("[");
                self.clear(ef);
                then_fn(self);
                self.clear(cond); self.goto(cond); self.raw("]");
                self.goto(ef); self.raw("[");
                ef_fn(self);
                self.clear(ef); self.goto(ef); self.raw("]");
                self.tfree(ef);
            }
        }
    }
    ...
    fn widen(&mut self, src: usize, from_w: usize, to_w: usize) -> usize {
        if from_w == to_w { return src; }
        let t = self.talloc_n(to_w);
        self.bfmove_n(src, t, from_w);
        for i in from_w..to_w { self.clear(t + i); }
    ...
    fn cmp_gt(&mut self, a: usize, b: usize) -> usize {
        let result = self.talloc(); self.clear(result);
        let flag   = self.talloc(); self.setv(flag, 1);
        self.goto(flag); self.raw("[");

        let bnz = self.is_nz(b);
        self.goto(bnz); self.raw("[");  // if b != 0
            self.dec(b, 1);
            let anz = self.is_nz(a);
            self.goto(anz); self.raw("[");
                self.dec(a, 1);
            self.clear(anz); self.goto(anz); self.raw("]");
            // else branch: a was 0 → a <= b, clear flag
            let ef_a = self.talloc(); self.setv(ef_a, 1);
            let anz2 = self.is_nz(a);
            self.goto(anz2); self.raw("[");
                self.clear(ef_a);
            self.clear(anz2); self.goto(anz2); self.raw("]");
            self.tfree(anz2);
            self.goto(ef_a); self.raw("[");
                self.clear(flag);
            self.clear(ef_a); self.goto(ef_a); self.raw("]");
            self.tfree(ef_a);
        self.clear(bnz); self.goto(bnz); self.raw("]");

        // else branch: b == 0
        let ef_b = self.talloc(); self.setv(ef_b, 1);
        let bnz2 = self.is_nz(b);
        self.goto(bnz2); self.raw("[");
            self.clear(ef_b);
        self.clear(bnz2); self.goto(bnz2); self.raw("]");
        self.tfree(bnz2);
        self.goto(ef_b); self.raw("[");
            let anz3 = self.is_nz(a);
            self.goto(anz3); self.raw("[");
                self.setv(result, 1);
            self.clear(anz3); self.goto(anz3); self.raw("]");
            self.tfree(anz3);
            self.clear(flag);
        self.clear(ef_b); self.goto(ef_b); self.raw("]");
        self.tfree(ef_b);
        self.tfree(bnz);
    ...
    fn cmp_lt(&mut self, a: usize, b: usize) -> usize { self.cmp_gt(b, a) }
    fn cmp_ge(&mut self, a: usize, b: usize) -> usize { let r = self.cmp_gt(b, a); self.negate(r) }
    fn cmp_le(&mut self, a: usize, b: usize) -> usize { let r = self.cmp_gt(a, b); self.negate(r) }
    fn cmp_eq(&mut self, a: usize, b: usize) -> usize { self.sub_from(b, a); self.tfree(b); self.negate(a) }
    fn cmp_ne(&mut self, a: usize, b: usize) -> usize { let r = self.cmp_eq(a, b); self.negate(r) }
    ...
    fn cmp_lt_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        if n == 1 { return self.cmp_lt(a, b); }
        // Accumulate from LSB up: result = (a[i] < b[i]) || (a[i] == b[i] && result_from_lower)
        let result = self.talloc(); self.clear(result);
        for i in 0..n {
            let a1 = self.talloc(); self.copy(a + i, a1);
            let b1 = self.talloc(); self.copy(b + i, b1);
            let a2 = self.talloc(); self.copy(a + i, a2);
            let b2 = self.talloc(); self.copy(b + i, b2);
            let lt = self.cmp_lt(a1, b1);
            let eq = self.cmp_eq(a2, b2);

            let new_result = self.talloc(); self.clear(new_result);
            let prev_copy = self.talloc(); self.copy(result, prev_copy);
            self.clear(result);

            // new_result = lt || (eq && prev_copy)
            let lt2 = lt; let nr = new_result;
            self.goto(lt2); self.raw("[");
                self.setv(nr, 1);
            self.clear(lt2); self.goto(lt2); self.raw("]");
            self.tfree(lt2);

            let eq2 = eq; let pc = prev_copy; let nr2 = new_result;
            self.goto(eq2); self.raw("[");
                self.goto(pc); self.raw("[");
                    self.setv(nr2, 1);
                self.clear(pc); self.goto(pc); self.raw("]");
            self.clear(eq2); self.goto(eq2); self.raw("]");
            self.tfree(eq2); self.tfree(prev_copy);

            self.bfmove(new_result, result);
            self.tfree(new_result);
        }
        self.tfree_n(a, n); self.tfree_n(b, n);
        result
    }

    fn cmp_gt_n(&mut self, a: usize, b: usize, n: usize) -> usize { self.cmp_lt_n(b, a, n) }
    fn cmp_le_n(&mut self, a: usize, b: usize, n: usize) -> usize { let r = self.cmp_gt_n(a, b, n); self.negate(r) }
    fn cmp_ge_n(&mut self, a: usize, b: usize, n: usize) -> usize { let r = self.cmp_lt_n(a, b, n); self.negate(r) }

    fn cmp_eq_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        if n == 1 { return self.cmp_eq(a, b); }
        // all bytes equal?
        let result = self.talloc(); self.setv(result, 1);
        for i in 0..n {
            let ai = self.talloc(); self.copy(a + i, ai);
            let bi = self.talloc(); self.copy(b + i, bi);
            let byte_eq = self.cmp_eq(ai, bi);
            // result = result && byte_eq
            let old = self.talloc(); self.bfmove(result, old);
            self.clear(result);
            self.goto(old); self.raw("[");
                self.goto(byte_eq); self.raw("[");
                    self.setv(result, 1);
                self.clear(byte_eq); self.goto(byte_eq); self.raw("]");
            self.clear(old); self.goto(old); self.raw("]");
            self.tfree(old); self.tfree(byte_eq);
        }
        self.tfree_n(a, n); self.tfree_n(b, n);
        result
    }
    ...
    fn cmp_ne_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        let r = self.cmp_eq_n(a, b, n); self.negate(r)
    }
    ...
    fn add_n(&mut self, dst: usize, src: usize, n: usize) {
        if n == 1 { self.add_to(src, dst); self.tfree(src); return; }
        for i in 0..n {
            let orig = self.talloc(); self.copy(dst + i, orig);
            self.add_to(src + i, dst + i);
            if i < n - 1 {
                let result_copy = self.talloc(); self.copy(dst + i, result_copy);
    ...
    fn sub_n(&mut self, dst: usize, src: usize, n: usize) {
        if n == 1 { self.sub_from(src, dst); self.tfree(src); return; }
        for i in 0..n {
            let orig = self.talloc(); self.copy(dst + i, orig);
            ...
            if i < n - 1 {
                ...
                let tgt = dst + i + 1;
                self.if_else(borrow, move |bf| { bf.dec(tgt, 1); }, None::<fn(&mut Self)>);
            } else {
                self.tfree(orig);
            }
        }
    ...
    fn is_nz_n(&mut self, base: usize, n: usize) -> usize {
        if n == 1 { return self.is_nz(base); }
        let r = self.talloc(); self.clear(r);
        for i in 0..n {
            let bc = self.talloc(); self.copy(base + i, bc);
            self.goto(bc); self.raw("[");
                self.setv(r, 1);
                self.clear(bc);
            self.goto(bc); self.raw("]");
            self.tfree(bc);
    ...
    fn arith_mul_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        if n == 1 { return self.arith_mul(a, b); }
        let r = self.talloc_n(n);
        for i in 0..n { self.clear(r + i); }
        // while a != 0: a -= 1; r += b_copy
        let flag = self.is_nz_n(a, n);
        self.goto(flag); self.raw("[");
            self.clear(flag);
            self.dec_n(a, n);
            let bc = self.talloc_n(n); self.copy_n(b, bc, n);
            self.add_n(r, bc, n);
            let flag2 = self.is_nz_n(a, n);
            self.bfmove(flag2, flag); self.tfree(flag2);
        self.goto(flag); self.raw("]");
        self.tfree(flag);
        self.tfree_n(a, n); self.tfree_n(b, n);
        r
    ...
    fn arith_div_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        if n == 1 { return self.arith_div(a, b); }
        let q = self.talloc_n(n);
        for i in 0..n { self.clear(q + i); }
        let flag = self.talloc(); self.setv(flag, 1);
        self.goto(flag); self.raw("[");
            self.clear(flag);
            let ta = self.talloc_n(n); let tb = self.talloc_n(n);
            self.copy_n(a, ta, n); self.copy_n(b, tb, n);
            let bgt = self.cmp_gt_n(tb, ta, n);  // b > a?
            let ge = self.negate(bgt);            // a >= b
            let b2 = b; let a2 = a; let q2 = q; let flag2 = flag;
            let n2 = n;
            self.goto(ge); self.raw("[");
                let ts = self.talloc_n(n2); self.copy_n(b2, ts, n2);
                self.sub_n(a2, ts, n2);
                self.inc_n(q2, n2);
                self.setv(flag2, 1);
            self.clear(ge); self.goto(ge); self.raw("]");
            self.tfree(ge);
        self.goto(flag); self.raw("]");
        self.tfree(flag);
        self.tfree_n(a, n); self.tfree_n(b, n);
    ...
    fn arith_mod_n(&mut self, a: usize, b: usize, n: usize) -> usize {
        if n == 1 { return self.arith_mod(a, b); }
        let flag = self.talloc(); self.setv(flag, 1);
        self.goto(flag); self.raw("[");
            self.clear(flag);
            let ta = self.talloc_n(n); let tb = self.talloc_n(n);
            ...
            let b2 = b; let a2 = a; let flag2 = flag; let n2 = n;
            self.goto(ge); self.raw("[");
                let ts = self.talloc_n(n2); self.copy_n(b2, ts, n2);
                self.sub_n(a2, ts, n2);
                self.setv(flag2, 1);
    ...
    fn arith_add(&mut self, a: usize, b: usize) -> usize { self.add_to(b, a); self.tfree(b); a }
    fn arith_sub(&mut self, a: usize, b: usize) -> usize { self.sub_from(b, a); self.tfree(b); a }

    fn arith_mul(&mut self, a: usize, b: usize) -> usize {
        let r = self.talloc(); self.clear(r);
        let tc = self.talloc();
        self.goto(a); self.raw("[");
            self.dec(a, 1);
            self.copy(b, tc);
            self.add_to(tc, r);
        self.goto(a); self.raw("]");
    ...
    fn arith_div(&mut self, a: usize, b: usize) -> usize {
        let q = self.talloc(); self.clear(q);
        let flag = self.talloc(); self.setv(flag, 1);
        self.goto(flag); self.raw("[");
            self.clear(flag);
            let ta = self.talloc(); let tb = self.talloc();
            self.copy(a, ta); self.copy(b, tb);
            let bga = self.cmp_gt(tb, ta);
            let ge = self.negate(bga);
            let b2 = b; let a2 = a; let q2 = q; let flag2 = flag;
            self.goto(ge); self.raw("[");
                let ts = self.talloc(); self.copy(b2, ts);
                self.sub_from(ts, a2); self.tfree(ts);
                self.inc(q2, 1);
                self.setv(flag2, 1);
            self.clear(ge); self.goto(ge); self.raw("]");
            self.tfree(ge);
    ...
    fn arith_mod(&mut self, a: usize, b: usize) -> usize {
        ...
        self.goto(flag); self.raw("[");
            ...
            let bga = self.cmp_gt(tb, ta);
            let ge = self.negate(bga);
            let b2 = b; let a2 = a; let flag2 = flag;
            self.goto(ge); self.raw("[");
                let ts = self.talloc(); self.copy(b2, ts);
                self.sub_from(ts, a2); self.tfree(ts);
                self.setv(flag2, 1);
    ...
    fn arr_read(&mut self, layout: &super::layout::CellLayout, idx_expr: &Expr) -> (usize, usize) {
        ...
        for k in 0..ew { self.clear(result + k); }
        for i in 0..arr_len {
            let ti = self.talloc(); self.copy(idx, ti);
            if i > 0 { self.dec(ti, i); }
            let eq = self.talloc(); self.setv(eq, 1);
            let tc = self.talloc(); self.copy(ti, tc);
            self.goto(tc); self.raw("[");
                self.clear(eq); self.clear(tc);
            self.goto(tc); self.raw("]");
            self.tfree(tc);
            let base_i = base + i * ew; let r = result; let ew2 = ew;
            self.goto(eq); self.raw("[");
                for off in 0..ew2 { self.copy(base_i + off, r + off); }
            self.clear(eq); self.goto(eq); self.raw("]");
    ...
    fn arr_write(&mut self, layout: &super::layout::CellLayout, idx_expr: &Expr, val: usize, val_w: usize) {
        let base = layout.base;
        let arr_len = layout.array_len();
        let ew = layout.elem_width();
        // Widen or truncate val to match element width
        let val = if val_w < ew { self.widen(val, val_w, ew) } else { val };
        let idx = self.eval_expr_1(idx_expr);
        for i in 0..arr_len {
            let ti = self.talloc(); self.copy(idx, ti);
            if i > 0 { self.dec(ti, i); }
            ...
            self.goto(tc); self.raw("]");
            self.tfree(tc);
            let base_i = base + i * ew; let v2 = val; let ew2 = ew;
            self.goto(eq); self.raw("[");
                for off in 0..ew2 {
    ...
    fn print_num(&mut self, cell: usize) {
        let v = self.talloc(); self.copy(cell, v);
        let h = self.talloc(); self.clear(h);
        let t = self.talloc(); self.clear(t);
        self.divconst(v, 100, h);
        self.divconst(v, 10, t);
        let hp = self.is_nz(h);
        let h2 = h;
        self.if_else(hp, |bf| {
            let d = bf.talloc(); bf.copy(h2, d); bf.inc(d, 48); bf.out(d); bf.tfree(d);
        }, None::<fn(&mut Self)>);
        self.tfree(hp);
        let ht = self.talloc(); self.clear(ht);
        let ch = self.is_nz(h); let ht2 = ht;
        self.goto(ch); self.raw("["); self.setv(ht2, 1); self.clear(ch); self.goto(ch); self.raw("]");
        self.tfree(ch);
        let ct = self.is_nz(t); let ht3 = ht;
        self.goto(ct); self.raw("["); self.setv(ht3, 1); self.clear(ct); self.goto(ct); self.raw("]");
        self.tfree(ct);
        let t2 = t;
        self.if_else(ht, |bf| {
            let d = bf.talloc(); bf.copy(t2, d); bf.inc(d, 48); bf.out(d); bf.tfree(d);
        }, None::<fn(&mut Self)>);
        self.tfree(ht);
        self.inc(v, 48); self.out(v);
        self.tfree(t2); self.tfree(h2); self.tfree(v);
    }
    ...
    fn divconst(&mut self, val: usize, div: u8, quot: usize) {
        ...
        self.goto(flag); self.raw("[");
            self.clear(flag);
            let tv = self.talloc(); let td = self.talloc();
            self.copy(val, tv); self.setv(td, div);
            let gt = self.cmp_gt(td, tv);
            let ge = self.negate(gt);
            let val2 = val; let quot2 = quot; let flag2 = flag; let div2 = div;
            self.goto(ge); self.raw("[");
    ...
    fn scan_num(&mut self, target: usize) {
        self.clear(target);
        let ch = self.talloc(); self.clear(ch); self.inp(ch);
        let lf = self.is_digit(ch);
        let t2 = target; let ch2 = ch;
        self.goto(lf); self.raw("[");
            self.mul10(t2);
            let dg = self.talloc(); self.copy(ch2, dg); self.dec(dg, 48);
            self.add_to(dg, t2); self.tfree(dg);
            self.clear(ch2); self.inp(ch2);
            self.clear(lf);
            let lf2 = self.is_digit(ch2);
            self.bfmove(lf2, lf); self.tfree(lf2);
        self.goto(lf); self.raw("]");
    ...
    fn is_digit(&mut self, ch: usize) -> usize {
        ...
        let t10 = self.talloc(); self.copy(ch, t10); self.dec(t10, 10);
        let c2 = self.is_nz(t10); self.tfree(t10);
        let t32 = self.talloc(); self.copy(ch, t32); self.dec(t32, 32);
        let c3 = self.is_nz(t32); self.tfree(t32);
        let r2 = r;
        self.goto(c1); self.raw("[");
            self.goto(c2); self.raw("[");
                self.goto(c3); self.raw("[");
                    self.setv(r2, 1);
                self.clear(c3); self.goto(c3); self.raw("]");
            self.clear(c2); self.goto(c2); self.raw("]");
        self.clear(c1); self.goto(c1); self.raw("]");
        self.tfree(c3); self.tfree(c2); self.tfree(c1);
        r
    }

    fn mul10(&mut self, cell: usize) {
        let tc = self.talloc(); self.copy(cell, tc); self.clear(cell);
        for _ in 0..10 {
            let t2 = self.talloc(); self.copy(tc, t2);
    ...
    fn mul10_n(&mut self, cell: usize, n: usize) {
        if n == 1 { self.mul10(cell); return; }
        let tc = self.talloc_n(n); self.copy_n(cell, tc, n);
        for i in 0..n { self.clear(cell + i); }
        for _ in 0..10 {
            let t2 = self.talloc_n(n); self.copy_n(tc, t2, n);
    ...
    fn scan_num_n(&mut self, target: usize, n: usize) {
        if n == 1 { self.scan_num(target); return; }
        for i in 0..n { self.clear(target + i); }
        let ch = self.talloc(); self.clear(ch); self.inp(ch);
        let lf = self.is_digit(ch);
        let ch2 = ch; let n2 = n;
        self.goto(lf); self.raw("[");
    ...
    fn divconst_n(&mut self, val: usize, n: usize, div: u32, quot: usize) {
        if n == 1 { self.divconst(val, div as u8, quot); return; }
        for i in 0..n { self.clear(quot + i); }
        let flag = self.talloc(); self.setv(flag, 1);
        self.goto(flag); self.raw("[");
            self.clear(flag);
            let tv = self.talloc_n(n); let td = self.talloc_n(n);
            self.copy_n(val, tv, n); self.set_const_n(td, div, n);
            let gt = self.cmp_gt_n(td, tv, n);  // div > val?
            let ge = self.negate(gt);            // val >= div
            let div2 = div; let val2 = val; let quot2 = quot; let flag2 = flag; let n2 = n;
            self.goto(ge); self.raw("[");
                let ts = self.talloc_n(n2); self.set_const_n(ts, div2, n2);
    ...
    fn print_num_n(&mut self, cell: usize, n: usize) {
        if n == 1 { self.print_num(cell); return; }
        // Work on a copy
        let v = self.talloc_n(n); self.copy_n(cell, v, n);

        // Determine divisors for the width
        let divisors: &[u32] = match n {
            2 => &[10000, 1000, 100, 10],
            4 => &[1000000000, 100000000, 10000000, 1000000, 100000, 10000, 1000, 100, 10],
            _ => &[],
        };
        ...
        for &div in divisors {
            ...
            let dg = self.talloc(); self.copy(quot, dg);
            digit_cells.push(dg);
            for i in 0..n { self.clear(quot + i); }
        }
        // Last digit is the remaining value (v mod 10)
        let last = self.talloc(); self.copy(v, last);
        ...
        let seen_nz = self.talloc(); self.clear(seen_nz);
        for (idx, &d) in digit_cells.iter().enumerate() {
            if idx == len - 1 {
                // Always print last digit
                let is_last = d;
                self.inc(is_last, 48); self.out(is_last); self.tfree(is_last);
            } else {
                let snz = seen_nz;
                let d2 = d;
                // if seen_nz or d != 0: print d
                let d_nz = self.is_nz(d);
                let should_print_flag = self.talloc(); self.clear(should_print_flag);
                let snz_copy = self.talloc(); self.copy(snz, snz_copy);
                self.goto(snz_copy); self.raw("[");
                    self.setv(should_print_flag, 1);
                self.clear(snz_copy); self.goto(snz_copy); self.raw("]");
                self.tfree(snz_copy);
                self.goto(d_nz); self.raw("[");
                    self.setv(should_print_flag, 1);
                self.clear(d_nz); self.goto(d_nz); self.raw("]");
                self.tfree(d_nz);

                let d3 = d2; let snz2 = snz;
                self.goto(should_print_flag); self.raw("[");
                    self.setv(snz2, 1);
                    let dc = self.talloc(); self.copy(d3, dc); self.inc(dc, 48); self.out(dc); self.tfree(dc);
                self.clear(should_print_flag); self.goto(should_print_flag); self.raw("]");
    ...
    fn eval_expr(&mut self, expr: &Expr) -> (usize, usize) {
        match expr {
            Expr::Int(n) => {
                let w = if *n < 256 { 1 } else if *n < 65536 { 2 } else { 4 };
            ...
            Expr::UnOp(UnOp::Neg, inner) => {
                ...
                for i in 0..w { self.clear(zero + i); }
                self.sub_n(zero, t, w);
                (zero, w)
            }
    ...
    fn eval_expr_1(&mut self, expr: &Expr) -> usize {
        ...
        if w > 1 {
            ...
            let lo = self.talloc(); self.bfmove(base, lo);
            ...
            for i in 1..w { self.clear(base + i); }
    ...
    fn eval_binop_w(&mut self, op: BinOp, a: usize, aw: usize, b: usize, bw: usize) -> (usize, usize) {
        let w = aw.max(bw);
        ...
        match op {
            BinOp::Add => { self.add_n(a, b, w); (a, w) }
            BinOp::Sub => { self.sub_n(a, b, w); (a, w) }
            ...
            BinOp::And => {
                ...
                let r2 = r;
                self.goto(anz); self.raw("[");
                    self.goto(bnz); self.raw("[");
                        self.setv(r2, 1);
                    self.clear(bnz); self.goto(bnz); self.raw("]");
                self.clear(anz); self.goto(anz); self.raw("]");
                self.tfree(anz); self.tfree(bnz);
                self.tfree_n(a, w); self.tfree_n(b, w);
                (r, 1)
            }
            BinOp::Or => {
                let r = self.talloc(); self.clear(r);
        ...
        match stmt {
            Stmt::Let { name, init, .. } => {
                if let Some(expr) = init {
                    ...
                    let v = if vw < layout.width { self.widen(v, vw, layout.width) } else { v };
            ...
            Stmt::Print(expr) => {
                let (v, vw) = self.eval_expr(expr);
                if vw == 1 { self.print_num(v); self.tfree(v); }
                else { self.print_num_n(v, vw); self.tfree_n(v, vw); }
                let sp = self.talloc(); self.setv(sp, 32); self.out(sp); self.tfree(sp);
            }
            Stmt::Putchar(expr) => {
    ...
    fn gen_while(&mut self, cond: &Expr, body: &[Stmt]) {
        let cc = self.talloc(); self.clear(cc);
        let sv = self.tsave();
        let (r, rw) = self.eval_expr(cond);
        let cond_val = if rw > 1 { let nz = self.is_nz_n(r, rw); self.tfree_n(r, rw); nz } else { r };
        self.bfmove(cond_val, cc); self.tfree(cond_val);
        self.trestore(sv);
        self.used.insert(cc.saturating_sub(self.temp_base()));

        self.goto(cc); self.raw("[");
        for s in body { self.gen_stmt(s); }
        let sv2 = self.tsave();
        let (r2, r2w) = self.eval_expr(cond);
        let cond_val2 = if r2w > 1 { let nz = self.is_nz_n(r2, r2w); self.tfree_n(r2, r2w); nz } else { r2 };
        self.clear(cc); self.bfmove(cond_val2, cc); self.tfree(cond_val2);
        self.trestore(sv2);
        self.used.insert(cc.saturating_sub(self.temp_base()));
        self.goto(cc); self.raw("]");
        self.tfree(cc);
    ...
    fn gen_if(&mut self, cond: &Expr, then_: &[Stmt], else_: Option<&[Stmt]>) {
        let sv = self.tsave();
        let cc = self.talloc(); let ef = self.talloc();
        self.clear(cc);
        let (r, rw) = self.eval_expr(cond);
        let cond_val = if rw > 1 { let nz = self.is_nz_n(r, rw); self.tfree_n(r, rw); nz } else { r };
        self.bfmove(cond_val, cc); self.tfree(cond_val);
        ...
        self.goto(cc); self.raw("[");
            self.clear(ef);
            for s in then_ { self.gen_stmt(s); }
        self.clear(cc); self.goto(cc); self.raw("]");
